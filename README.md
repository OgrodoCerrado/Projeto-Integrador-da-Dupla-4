...
Principais funcionalidades desenvolvidas: Liberdade de decisão do usuário em decidir que opções ele quer, entrega do nome de cada receita, apresentação de opções para cada seleção

Contéudo em C utilizados:Estruturas de repetição(do, for e while), estruturas de decisão(switch case e if else), structs, modularização, listas, int, printf, ponteiros, biblioteca stdio.h e define

Nomes dos autores: Igor Pechir Ramos e Italo Amorim Figueiredo

Estrutura de pastas:Elas estão divididas da forma que foi pedida no roteiro do projeto 3, seguindo o src, include, docs e backup
